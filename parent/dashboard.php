                ...
                <?php while ($student = $students->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            } elseif (!empty($student['requested_driver_id']) && $student['driver_request_status'] === 'pending') {
                                $driver = $conn->query("SELECT full_name FROM users WHERE id = {$student['requested_driver_id']}")->fetch_assoc();
                                echo htmlspecialchars($driver['full_name']) . ' (Pending)';
                            } elseif (!empty($student['requested_driver_id']) && $student['driver_request_status'] === 'declined') {
                                $driver = $conn->query("SELECT full_name FROM users WHERE id = {$student['requested_driver_id']}")->fetch_assoc();
                                echo htmlspecialchars($driver['full_name']) . ' (Declined)';
                            } else {
                                echo 'Not assigned';
                            }
                            ?>
