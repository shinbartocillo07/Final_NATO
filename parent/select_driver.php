...
if (isset($_POST['select_driver'])) {
    ...
    if ($student_check->num_rows > 0) {
        $conn->query("UPDATE students SET requested_driver_id = $driver_id, driver_request_status = 'pending' WHERE id = $student_id");
        $message = 'Driver request sent successfully. The driver will review it shortly.';
    } else {
            ...
            <tbody>
                ...
                while ($student = $students->fetch_assoc()):
                    ...
                    if ($student['driver_id']) {
                        $driverInfo = $conn->query("SELECT full_name FROM users WHERE id = {$student['driver_id']}")->fetch_assoc();
                        $currentDriver = $driverInfo['full_name'];
                    } elseif (!empty($student['requested_driver_id'])) {
                        $driverInfo = $conn->query("SELECT full_name FROM users WHERE id = {$student['requested_driver_id']}")->fetch_assoc();
                        $currentDriver = $driverInfo['full_name'];
                    }
                ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($currentDriver ?? 'Not assigned'); ?></td>
                        <td><?php echo htmlspecialchars(ucfirst($student['driver_request_status'] ?? 'none')); ?></td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
        </table>

        <h2>Request Driver for Student</h2>
        <form method="POST">
            ...
            <button type="submit" name="select_driver">Request Driver</button>
        </form>
    </div>
