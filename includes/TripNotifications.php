...
class TripNotifications {
    ...
    /**
     * Notify parent when a driver accepts the student request
     */
    public function notifyParentDriverAccepted($studentId, $driverId) {
        $student = $this->getStudentDetails($studentId);
        $driver = $this->getDriverDetails($driverId);
        $parent = $this->getParentDetails($student['parent_id']);

        if ($parent) {
            $title = "Driver Request Accepted";
            $message = "{$driver['full_name']} has accepted the request to transport {$student['name']}";
            $data = [
                'student_name' => $student['name'],
                'grade' => $student['grade'],
