...
class NotificationManager {
    ...
    public function __construct($connection) {
        // The database connection is used for saving and reading notifications.
        $this->conn = $connection;
        // Load email server settings from the shared mail configuration file.
     ...
     * 2. Look up user email address.
     * 3. Send an email if an address exists.
     */
    public function send($userId, $title, $message, $type, $data = []) {
        $this->saveToDatabase($userId, $title, $message, $type, $data);

        // Look up the recipient so we can send email as a fallback channel.
