...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Read and sanitize user-entered values.
    ...
    // Prevent duplicate accounts by checking username and email uniqueness.
    ...
    if ($stmt->get_result()->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            // Additional driver details are stored in a separate table.
            if ($role == 'driver') {
                $license = sanitize($_POST['license']);
                $vehicle_type = sanitize($_POST['vehicle_type']);
                $vehicle_plate = sanitize($_POST['vehicle_plate']);

                $stmt = $conn->prepare("INSERT INTO drivers (user_id, license_number, vehicle_type, vehicle_plate, status) VALUES (?, ?, ?, ?, 'pending')");
                $stmt->bind_param("isss", $user_id, $license, $vehicle_type, $vehicle_plate);
                $stmt->execute();

                // Only drivers should see this pending message.
                $success = 'Registration successful! Your driver account is pending admin approval. You will receive an email when your account is approved.';
            } else {
